...
void stabilization_opticflow_update(struct opticflow_result_t *result)
{
  ...
  float cmd_phi;
    float cmd_theta;
    cmd_phi=((float)opticflow_stab.phi_pgain) * err_vx / 100
            + ((float)opticflow_stab.phi_igain) * opticflow_stab.err_vx_int;
    cmd_theta=-(((float)opticflow_stab.theta_pgain) * err_vy / 100
                      + ((float)opticflow_stab.theta_igain) * opticflow_stab.err_vy_int);

    opticflow_stab.cmd.phi   = cmd_phi;
    opticflow_stab.cmd.theta = cmd_theta;*/
