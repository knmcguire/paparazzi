...
static void dummy(dwDevice_t* dev){
  ;
}
...
void dwSetEUI(dwDevice_t* dev, uint64_t eui)
{
    dwSpiWrite(dev, EUI, 0, (uint8_t*)&eui, 8);
}

uint64_t dwGetEUI(dwDevice_t* dev) {
    uint64_t result;
    dwSpiRead(dev, EUI, 0, (uint8_t*)&result, 8);
    return result;
}
...
void dwSetDefaults(dwDevice_t* dev) {
	if(dev->deviceMode == TX_MODE) {
		...
		dwSuppressFrameCheck(dev, false);//false
    ...
    dwInterruptOnReceiveTimeout(dev, true);//true
...
float dwGetFirstPathPower(dwDevice_t* dev) {
	...
	float estFpPwr = 10.0f * log10(((float)f1 * (float)f1 + (float)f2 * (float)f2 + (float)f3 * (float)f3) / ((float)N * (float)N)) - A;
	if(estFpPwr <= -88) {
		return estFpPwr;
	} else {
...
float dwGetReceivePower(dwDevice_t* dev) {
	...
	float estRxPwr = 10.0f * log10(((float)C * (float)twoPower17) / ((float)N * (float)N)) - A;
	if(estRxPwr <= -88) {
		return estRxPwr;
	} else {
		// approximation of Fig. 22 in user manual for dbm correction
...
void (*_handleError)(dwDevice_t* dev) = dummy;
void (*_handleReceiveFailed)(dwDevice_t* dev) = dummy;
void (*_handleReceiveTimestampAvailable)(dwDevice_t* dev) = dummy;
