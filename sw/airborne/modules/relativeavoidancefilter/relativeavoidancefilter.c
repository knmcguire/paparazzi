...
void rafilter_periodic(void)
{
	...
	// 		ekf_filter_reset(vector_get(&ekf_vec,i), X, P);
	...
	// 	ekf_filter_predict(vector_get(&ekf_vec,i), U);
	// 	ekf_filter_update(vector_get(&ekf_vec,i), Y);

	// 	// Get the outputs of the ekf into the main
	// 	ekf_filter_get_state(vector_get(&ekf_vec,i), X, P);
